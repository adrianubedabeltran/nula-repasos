...
async function generarInforme(notes, obraTitle) {
  try {
    ...
    doc.text("REPASOS PENDIENTES — " + new Date().toLocaleDateString("es-ES"), W - margin, y, { align: "right" });
    y += 4;
    ...
    if (pendientes.length === 0) {
      ...
      doc.save("repasos-pendientes.pdf");
      return;
    }
    ...
    for (const [responsable, repasos] of Object.entries(porResponsable)) {
      ...
      for (const note of repasos) {
        ...
        const lines = doc.splitTextToSize(note.comentario || "", contentW - 40);
        const blockH = Math.max(14, 5 + lines.length * 4.5 + 4);
        checkPage(blockH + 3);
        ...
        doc.text(note.fecha || "", W - margin - 3, y + 5, { align: "right" });
    ...
    for (let p = 1; p <= total; p++) {
      ...
      doc.text("Nula.Studio — Repasos pendientes", margin, 291);
